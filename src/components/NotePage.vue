<template>
  <div class="container">
    ...
    <span class="breadcrumb">
      <Icon type="md-folder" />
      <span class="path-tag">笔记</span>
      <span v-for="item of pathList" :key="item" class="path-tag">
        <span class="path-tag-split">/</span>
        {{ item }}
      </span>
    </span>
    <div class="header">
  ...
  methods: {
    requestNoteContent() {
      axios
        .post(
          this.$apiUrl +
            `noteInfo/getNoteContent?target=${this.noteTaget}&name=${this.noteName}`
        )
        .then(res => {
          const {
            status,
